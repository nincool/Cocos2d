...
bool CGameSocket::Create(const char* pszServerIP, int nServerPort, int nBlockSec, bool bKeepAlive /*= FALSE*/)
{
	...
	{
		closeSocket();
		CHrLog::Instance().Log(_HERROR, "TCP", "IP地址格式错误 Error!");
		return false;
	}
...
bool CGameSocket::ReceiveMsg(void* pBuf, int& nSize)
{
	...
	// 检查是否有一个消息(小于4则无法获取到消息长度) Modify By Hr ->4
	if (m_nInbufLen < 4) {
	...
	int nPackSize = (*(int*)m_bufInput) + 4;//协定这个大小不包含前面的4个字节
	...
	if (nPackSize > m_nInbufLen) {
		...
		if (!recvFromSock() || nPackSize > m_nInbufLen) {	// 这个m_nInbufLen已更新
			return false;
		}
	}
