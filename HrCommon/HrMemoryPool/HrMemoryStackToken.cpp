...
CHrMemoryStackToken::CHrMemoryStackToken( int nBlockSize)
{
	m_nBlockInSide = 0;
	m_nBlockOutSide = 0;
...
CHrMemoryStackToken::~CHrMemoryStackToken()
{
	...
	{
		HRLOG(_HDEBUG, "HPOOL", "Hr Memory Stack:lost:%d * %d\n", m_nBlockSize, m_nBlockOutSide);
	}
...
void* CHrMemoryStackToken::Malloc( uint64 nSize, uint& nAllBlockCount, uint& nMemoryUse )
{
	...
	if (nSize >= HR_MEM_BLOCK_MAX_SAVE_BLOCK_SIZE)
	{
		...
		{
			HRLOG(_HDEBUG, "HPOOL", "HrMemoryStackToken Malloc Error without memory pool!!! nSize[%d]", nSize);
		}
	}
	...
	if (HR_MEM_BLOCK_SIZE( nSize ) < m_nBlockSize)
	{
		...
		if (m_pFirstSon == nullptr)
		{
			...
			else
			{
				...
				*/
				HRLOG(_HDEBUG, "HPOOL", "HrMemoryStackToken Malloc Error!!! m_nBlockSize[%d]", m_nBlockSize);
				CCASSERT(nullptr, "HrMemoryStackToken Malloc Error!!");
			}
		}
		else
		{
	...
	else
	{
		...
		if (m_pNext == nullptr)
		{
			m_pNext = new CHrMemoryStackToken( m_nBlockSize * 2 );
		}
		if (m_pNext != nullptr)
		{
...
void CHrMemoryStackToken::PrintStack()
{
	if (m_nBlockInSide + m_nBlockOutSide > 0)
	{
		...
		HRLOG(_HDEBUG, "HPOOL", " [%ld]stack:all=%d, out=%d, in=%d\n"
